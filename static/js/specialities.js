document.addEventListener("htmx:afterSwap", (e) => {
  ...
  if (e.detail.target.id === "doctor-info") {
    const trigger = e.detail.requestConfig.elt,
      skill = trigger.dataset.skill.toLowerCase(),
      docSymptoms = document.getElementById("symptoms"),
      docProcedures = document.getElementById("procedures");
    console.log(skill)

    if (skill && specialities[skill]) {
      docSymptoms.textContent = specialities[skill].symptoms;
      docProcedures.textContent = specialities[skill].procedures;
    };
  };
});
